...
def drawAddAlarm():
    ...
    alarmTitleFrame.pack(ipady="10")
    alarmTitleLbl = createLabel(alarmTitleFrame, "Alarm Name: ").grid(row=0, column=0, ipady="10")
    alarmTitleEnt = Entry(alarmTitleFrame, width="30", textvariable=alarmTitleVar).grid(row=0, column=1, ipady="10")

    #Choose time section
    global ampmVar, hourVar, minVar, secVar
    ampmVar = IntVar()
    hourVar = StringVar()
    minVar = StringVar()
    secVar = StringVar()
    ...
    colonLbl2 = createLabel(chooseTimeFrame, ":")
    secSbx = Spinbox(chooseTimeFrame, bg="Black", fg="White", width="5", textvariable=secVar, from_=0, to=59, values=constructMinSbxValues())

    hourSbx.grid(row=0, column=1, ipady="10")
    colonLbl.grid(row=0, column=2, ipady="10")
    minSbx.grid(row=0, column=3, ipady="10")
    colonLbl2.grid(row=0, column=4, ipady="10")
    secSbx.grid(row=0, column=5, ipady="10")
    amRad = Radiobutton(chooseTimeFrame, text="AM", variable=ampmVar, value=1).grid(row=0, column=6)
    pmRad = Radiobutton(chooseTimeFrame, text="PM", variable=ampmVar, value=2).grid(row=0, column=7)

    #Choose weekdays frame
    global weekday
    weekday = IntVar()
    chooseWeekdayFrame = Frame(root, width="200", bg="Black")
    chooseWeekdayFrame.pack(ipady="10")
    chooseWeekdayLbl = createLabel(chooseWeekdayFrame, "Repeat on: ")
    chooseWeekdayLbl.grid(row=0, column=0, ipady="10")
    chooseWeekdayListbox = Listbox(chooseWeekdayFrame, width="30", height="7",bg="Black", fg="White", selectmode=SINGLE)
    chooseWeekdayListbox.insert(0, "Mondays")
    chooseWeekdayListbox.insert(1, "Tuesdays")
    chooseWeekdayListbox.insert(2, "Wednesdays")
    chooseWeekdayListbox.insert(3, "Thursdays")
    chooseWeekdayListbox.insert(4, "Fridays")
    chooseWeekdayListbox.insert(5, "Saturdays")
    chooseWeekdayListbox.insert(6, "Sundays")
    chooseWeekdayListbox.insert(7, "Don't repeat")
    chooseWeekdayListbox.grid(row=1, column=0, ipady="10")

    #Intervals frame
    global intervalnVar, inthourVar, intminVar, intsecVar
    ...
    inthourSbx = Spinbox(intervalFrame, textvariable=inthourVar, bg="Black", fg="White", width="5", from_=0, to=12, values=constructIntHourSbxValues())
    ...
    intcolonLbl2 = createLabel(intervalFrame, ":")
    intsecSbx =  Spinbox(intervalFrame, textvariable=intsecVar, bg="Black", fg="White", width="5", from_=0, to=59, values=constructMinSbxValues())
    inthourSbx.grid(row=1, column=1)
    intcolonLbl.grid(row=1, column=2)
    intminSbx.grid(row=1, column=3)
    intcolonLbl2.grid(row=1, column=4)
    intsecSbx.grid(row=1, column=5)

    #Alert type frame
    global alertTypeVar
    alertTypeVar = IntVar()
    alertFrame = Frame(root, width="200", bg="Black")
    alertFrame.pack(ipady="10")
    ...
    currentWidgets.append(colonLbl2)
    currentWidgets.append(secSbx)
    currentWidgets.append(amRad)
    currentWidgets.append(pmRad)
    currentWidgets.append(chooseWeekdayFrame)
    currentWidgets.append(chooseWeekdayLbl)
    currentWidgets.append(chooseWeekdayListbox)
    ...
    currentWidgets.append(intcolonLbl2)
    currentWidgets.append(intsecSbx)
